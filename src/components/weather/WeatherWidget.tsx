...
export function WeatherWidget() {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200 dark:border-slate-700 text-sm">
      {getWeatherIcon()}
      <div className="flex items-center gap-1.5">
        <Thermometer className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
        <span className="text-white font-medium">{weather.temp}°F</span>
        <span className="text-slate-500 dark:text-slate-400 hidden sm:inline">• {weather.city}</span>
      </div>
    </div>
  );
}
